...
public class FullHouseRule extends AbstractPokerRule<Card.Value> {
    ...
    public boolean applicable(Hand hand) {
        return isValidHand(hand)
                ...
                .filter(groupedByValue -> groupedByValue.size() == 2 && groupedByValue.values().stream().anyMatch(cards -> cards.size() == 3) && groupedByValue.values().stream().anyMatch(cards -> cards.size() == 2))
                .isPresent();
    }
    ...
    public HandResult<Card.Value> rank(Hand hand) {
        return isValidHand(hand).map(this::groupByValue).map(groupedByValue -> {
